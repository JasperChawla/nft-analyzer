...
# Fetch top 100 collections (can expand to more pages if needed)
...
output_path = os.path.join(output_dir, "pfp.csv")

df = pd.DataFrame(collection_list)
# Change this line - add index=True to include the DataFrame index in the CSV
df.to_csv(output_path, index=True)
print(f"Saved {len(df)} cleaned PFP collections to {output_path}")